<template>
  <div class="fashionstatistic-page">
    <ali-head :showBack="true" :pageTitle="translate(81, lang)" :showDrawer="true"></ali-head>
    <scroller :on-refresh="refresh">
      <div class="interval"></div>
      <div class="result-text">
        <p class="title">{{translate(83, lang)}}</p>
        <p class="title">{{translate(86, lang)}}</p>
  ...
  methods: {
    ...
    getFashionImgList (fn) {
      ...
      this.$axios.get(urls.getFashionImgList).then((res) => {
        ...
        res.data.map((item) => {
          this.resultList.push({id: item.id, score: item.gradeDetailList.length})
        })
